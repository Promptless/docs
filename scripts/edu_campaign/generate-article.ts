...
function createPR(articleContent: string, keyword: string): string {
  ...
  const date = todayISO();
  const branchName = `content-farm/${date}-${slug}`;
  ...
  fs.writeFileSync(absoluteFilePath, articleContent + "\n");

  // Make sure we're starting from a clean main
...
🤖 Generated with Claude Opus 4.6`;
  ...
  const prUrl = exec(
    `gh pr create --title "content: ${title}" --body ${JSON.stringify(prBody)} --base main`
  );
