...
graph TB
    ...
    %% External Services - Made taller to show bidirectional flow
    subgraph "Corporate Network"
        direction TB
        GHE[GitHub Enterprise<br/>github.corp.com<br/><br/>📤 Sends Webhooks<br/>📥 Receives API Calls]
        SLACK[Slack Workspace<br/>corp.slack.com<br/><br/>📤 Sends Webhooks<br/>📥 Receives API Calls]
    end
    ...
    subgraph "AWS VPC"
        ...
        subgraph "EKS Cluster"
            subgraph "Promptless Namespace"
                API[Promptless Backend API<br/>K8s Deployment]
                DASH[Promptless Frontend Dashboard<br/>K8s Deployment]
                WORKER[Worker Processes<br/>K8s Deployment]
            end
        end
        ...
        subgraph "VPC Endpoints"
            ...
            VPCE_RDS[RDS VPC Endpoint]
            VPCE_BEDROCK[Bedrock VPC Endpoint]
        end
    end

    %% AWS Bedrock
    subgraph "AWS Bedrock"
        BEDROCK[Claude 3 Sonnet<br/>LLM Service]
    end
    ...
    %% Bidirectional external integrations
    %% Webhooks from external services (incoming)
    GHE -->|Webhooks<br/>Push/PR Events| API
    SLACK -->|Webhooks<br/>Slash Commands| API
    
    %% API calls to external services (outgoing)
    API -->|API Calls<br/>Repo Access| GHE
    API -->|API Calls<br/>Message Posting| SLACK
    WORKER -->|API Calls<br/>Code Analysis| GHE
    WORKER -->|API Calls<br/>Notifications| SLACK
