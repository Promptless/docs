...
An interesting paper [Chen et al. 2025](https://arxiv.org/html/2503.15231v1) tested what happens when you strip different parts of API docs before feeding them to an LLM. Removing code examples caused pass rates to collapse from 66–82% down to 22–39%. Removing parameter descriptions had a much smaller effect.

As for what examples to include, research on few-shot learning ([Xiao & Zhao 2025](https://arxiv.org/html/2505.19426v2)) shows that diverse examples outperform similar ones, especially as task complexity increases — the accuracy gain from diversity is 10.72% on hard tasks versus 5.11% on easy ones. Diverse examples also generalize better when an agent’s query differs from the exact scenario an example was designed for.

Should you include negative examples? You can, but tread carefully — the research is mixed.

- **1. The key is contrastive.** There is extensive research showing that contrastive learning improves LLM performance ([Mo et al. 2024](https://arxiv.org/abs/2402.11254)), but the operative word is "contrastive." A negative example needs a paired positive to produce that benefit; a negative example in isolation is more likely to hurt.

- **2. Subtle wrong beats obviously wrong.** Examples near the model’s decision boundary are the most effective ([Di et al. 2026](https://arxiv.org/html/2602.03516v2)). If you include negative examples, "subtly wrong" or "plausibly correct but wrong" outperforms "blatantly wrong." Trivial formatting errors don’t help and can backfire if the model imitates them.

- **3. Add comments inside your code snippets.** Research ([Zhao et al. 2025](https://arxiv.org/abs/2405.19874)) found that instruction-tuned LLMs sometimes treat demonstration examples as additional tasks to complete rather than as guidance. More broadly, examples from one part of your docs can contaminate an unrelated context elsewhere. A large-scale analysis of 673 agent skills ([Carey 2026](https://agentskillreport.com/), thank you [Dachary](https://www.linkedin.com/in/dachary/) for the feedback on this!) found contamination patterns including shell syntax appearing in JavaScript outputs, templates reproduced verbatim in unrelated contexts, and architectural conventions leaking into tasks where they don’t belong. Inline comments help the model distinguish examples from instructions and positive cases from negative ones.

**What to do**: For your most critical API reference and how-to pages, invest in a small, curated set of examples rather than as many as you can find. Prioritize diversity over volume, especially for complex workflows. Add inline comments to make the intent of each snippet explicit — whether it is a correct example, a near-miss, or a step in an instruction sequence — rather than relying solely on the surrounding markdown. Support tickets are often a great source for near-miss cases that real users actually encounter.

The main downside of many examples is maintenance overhead. (Or, you can try Promptless 😊)

Not many teams do directory-level examples yet, but top dev tools are trending in this direction. Some maintain dedicated examples directories in their repos (for instance [Vercel](https://github.com/vercel/vercel/tree/main/examples)). Others publish cookbooks (for instance [OpenAI](https://github.com/openai/openai-cookbook)). At this scale, diversity matters most — covering varied parameter combinations and use cases. Negative examples don’t translate well to directory scale, where a misconstrued pattern reproduced across a codebase can cause widespread harm.
...
Bhope, R. A., Venkateswaran, P., Jayaram, K. R., Isahagian, V., Muthusamy, V., & Venkatasubramanian, N. (2025). OptiSeq: Optimizing Example Ordering for In-Context Learning. [link](https://arxiv.org/html/2501.15030v1)
...
Tran, N., Litman, D., Pierce, B., Correnti, R., & Matsumura, L. C. (2025). Improving In-context Learning Example Retrieval for Classroom Discussion Assessment with Re-ranking and Label Ratio Regulation. Proceedings of the 20th Workshop on Innovative Use of NLP for Building Educational Applications (BEA 2025). [link](https://aclanthology.org/2025.bea-1.54.pdf)
