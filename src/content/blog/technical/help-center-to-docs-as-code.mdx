...
At some point, most teams with a developer-facing product ask the same question: should we move from our help center to docs-as-code? The trigger is usually the same: developers won't update articles in a browser-based editor, documentation drifts from the product, and someone suggests that putting docs in Git will fix it. It might. But the answer depends on specifics that the question usually skips.
...
That said, this is a weaker reason to stay put than it used to be. AI coding tools are making it easier for non-technical contributors to interact with Git-based workflows using natural language. The general direction is that people increasingly interact with computers through language rather than through specialized interfaces. Choosing a help center because your non-technical contributors don't know Git today is a bet that this won't change — which is increasingly a bad bet to make.
...
Regardless of which system you're in, you need a way to detect when [documentation has drifted](/blog/technical/documentation-drift-detection-problem) from the product it describes. That's a monitoring problem that exists on both sides of the migration. The teams with accurate documentation treat it as [a continuous monitoring task](/blog/technical/how-teams-keep-docs-up-to-date-with-promptless), not a periodic writing project.
