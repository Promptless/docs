...
When the harness fails, the agent breaks visibly: sessions stall, retries loop endlessly, state gets corrupted. When context fails, nothing breaks. The agent runs exactly as designed, does the wrong thing confidently, and gives no signal that anything is off.
