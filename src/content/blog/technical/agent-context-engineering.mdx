...
When the harness fails, the agent breaks visibly: sessions don't complete, retries loop, sub-agents contradict each other, state gets corrupted. When context fails, nothing breaks. The agent runs exactly as designed, does the wrong thing confidently, and gives no signal that anything is off.
