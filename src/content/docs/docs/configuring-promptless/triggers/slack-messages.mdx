...
1. **Acknowledgment**: Promptless sends a progress notification (for message actions) or acknowledges your request in the thread (for @mentions)
2. **Thread Analysis**: Promptless reads the entire thread where it was triggered
3. **Context Gathering**: The system analyzes text, images, and file attachments in the thread
4. **Documentation Suggestions**: Promptless creates suggestions based on the conversation content and notifies you when ready. If no documentation update is needed, Promptless lets you know instead of staying silent.
5. **Follow-up Editing**: Continue editing suggestions within the same thread using @Promptless mentions
