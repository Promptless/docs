...
Promptless creates branches with the `promptless/` prefix (e.g., `promptless/update-api-docs`). This consistent naming convention makes it easy to identify Promptless-created branches and configure branch protection rules.

<Tip>
If your repository uses branch protection rules or rulesets that restrict branch creation, ensure the `promptless/*` pattern is allowed. This lets Promptless create documentation branches while maintaining your protection policies.
</Tip>
