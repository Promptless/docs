...
- **Doc Detective** — Enable or disable Doc Detective testing and set the config path

**Settings that require support:**

Platform, repository, directories, and branch are scope-defining fields that cannot be edited in the dashboard. To change these settings, contact [help@gopromptless.ai](mailto:help@gopromptless.ai).
